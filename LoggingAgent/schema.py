...
class LogAgentOutput(BaseModel):
    ...
    logql_query: str = Field(description="Best-fitting LogQL query to be executed on Grafana based on user's request")
    from_time: str = Field(description="Timestamp in format YYYY-MM-DD HH:MM:SS", default="")
    to_time: str = Field(description="Timestamp in format YYYY-MM-DD HH:MM:SS", default="")